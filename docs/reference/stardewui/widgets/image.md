...
!!! tip "Framework View"
    ...
    The tag name of this view is `<image>`.
## Definition
...
| [Fit](#fit)<br/>`fit` | How to fit the image in the content area, if sizes differ. | 
...
| [HorizontalAlignment](#horizontalalignment)<br/>`horizontal-alignment` | Specifies where to align the image horizontally if the image width is different from the final layout width. | 
...
| [Rotation](#rotation)<br/>`rotation` | Rotation to apply to the image. | 
| [Scale](#scale)<br/>`scale` | Scale to apply to the image. | 
...
| [ShadowOffset](#shadowoffset)<br/>`shadow-offset` | Offset to draw the sprite shadow, which is a second copy of the [Sprite](image.md#sprite) drawn entirely black. Shadows will not be visible unless [ShadowAlpha](image.md#shadowalpha) is non-zero. | 
| [Sprite](#sprite)<br/>`sprite` | The sprite to draw. | 
| [SpriteEffects](#spriteeffects)<br/>`sprite-effects` | Sprite effects to apply, such as horizontal or vertical flipping. | 
...
| [Tint](#tint)<br/>`tint` | Tint color (multiplier) to apply when drawing. | 
...
| [VerticalAlignment](#verticalalignment)<br/>`vertical-alignment` | Specifies where to align the image vertically if the image height is different from the final layout height. | 
