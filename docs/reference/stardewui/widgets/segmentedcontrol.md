...
!!! tip "Framework View"
    ...
    The tag name of this view is `<segments>`.
## Definition
...
| [Balanced](#balanced)<br/>`balanced` | Whether to balance the segments so that each has the same width. | 
...
| [Children](#children)<br/>`children` | Child views to display in this layout. | 
...
| [Highlight](#highlight)<br/>`highlight` | Highlight sprite to draw beneath the selected segment. | 
| [HighlightTint](#highlighttint)<br/>`highlight-tint` | Tint color with which to draw the [Highlight](segmentedcontrol.md#highlight) sprite. | 
| [HighlightTransition](#highlighttransition)<br/>`highlight-transition` | Transition animation to use for moving the [Highlight](segmentedcontrol.md#highlight) when a new segment is selected. | 
| [HorizontalContentAlignment](#horizontalcontentalignment)<br/>`horizontal-content-alignment` | Horizontal alignment for each frame's content. | 
| [HoveredSubject](../view.md#hoveredsubject)<br/>`hovered-subject` | When Lookup Anything (Pathoschild.LookupAnything) is loaded, this Object or NPC subject is given to lookup anything for it's menu.<br><span class="muted" markdown>(Inherited from [View](../view.md))</span> | 
...
| [ScreenRead](../view.md#screenread)<br/>`screen-read` | When a screen reader mod (shoaib.stardewaccess) is loaded, this element will be announced by the screen reader using this value.<br><span class="muted" markdown>(Inherited from [View](../view.md))</span> | 
...
| [SelectedIndex](#selectedindex)<br/>`selected-index` | Index of the segment (element of [Children](segmentedcontrol.md#children)) that is considered to be selected. | 
| [SelectionSound](#selectionsound)<br/>`selection-sound` | Sound cue to play when a new item is selected. | 
| [Separator](#separator)<br/>`separator` | Separator sprite to draw between segments. | 
| [SeparatorTint](#separatortint)<br/>`separator-tint` | Tint color with which to draw the [Separator](segmentedcontrol.md#separator) sprite. | 
| [SeparatorWidth](#separatorwidth)<br/>`separator-width` | Width to draw the [Separator](segmentedcontrol.md#separator), if any is specified. | 
...
| [VerticalContentAlignment](#verticalcontentalignment)<br/>`vertical-content-alignment` | Vertical alignment for each segment's content. | 
