...
!!! tip "Framework View"
    ...
    The tag name of this view is `<tab>`.
## Definition
...
### Properties
 ...
 | Name | Description |
| --- | --- |
| [Active](#active)<br/>`active` | Whether or not the tab is considered active (selected). | 
| [ActiveOffset](#activeoffset)<br/>`active-offset` | The drawing offset to apply when the tab is [Active](tab.md#active). | 
...
| [Background](#background)<br/>`background` | Background image to draw behind the tab's [Content](tab.md#content), which provides the tab appearance. | 
| [BackgroundRotation](#backgroundrotation)<br/>`background-rotation` | Rotation of the [Background](tab.md#background); does not apply to [Content](tab.md#content). | 
...
| [Content](#content)<br/>`content` | Content to draw inside the tab's border. | 
...
| [ContentMargin](#contentmargin)<br/>`content-margin` | Margin to apply to the frame containing the [Content](tab.md#content), i.e. distance between the tab's visual border and the inner image, text, etc. | 
...
| [Group](#group)<br/>`group` | The selection group, if any, to which this tab belongs. | 
| [GroupKey](#groupkey)<br/>`group-key` | The unique key per [Group](tab.md#group) that identifies this tab. | 
